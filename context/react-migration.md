...
The old vanilla JS files in `/public` are kept for reference but no longer used when running `npm run dev` or production builds. They can be removed once confidence in the React version is established.

## Rollback Plan

If issues arise, the server automatically falls back to serving legacy files if `dist/` folder doesn't exist. Simply:
1. Delete `dist/` folder
2. Restart server
3. Server will serve from `public/` folder

To permanently rollback:
1. Revert package.json scripts
2. Remove React dependencies
3. Remove `src/` folder
4. Keep server code unchanged
